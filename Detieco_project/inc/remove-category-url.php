...
function remove_category_url_permastruct() {
	...
	if ( 3.4 <= $wp_version ) {
		$wp_rewrite->extra_permastructs['category']['struct'] = '/infocentr/%category%';
	} else {
		$wp_rewrite->extra_permastructs['category'][0] = '/infocentr/%category%';
	}
}
...
function remove_category_url_rewrite_rules( $category_rewrite ) {
	...
	foreach ( $categories as $category ) {
		...
		$category_rewrite[ 'infocentr/(' . $category_nicename . ')/(?:feed/)?(feed|rdf|rss|rss2|atom)/?$' ] = 'index.php?category_name=$matches[1]&feed=$matches[2]';
		$category_rewrite[ 'infocentr/(' . $category_nicename . ')/page/?([0-9]{1,})/?$' ] = 'index.php?category_name=$matches[1]&paged=$matches[2]';
		$category_rewrite[ 'infocentr/(' . $category_nicename . ')/?$' ] = 'index.php?category_name=$matches[1]';
	}

	// Redirect support from Old Category Base
	$old_category_base = get_option( 'category_base' ) ? get_option( 'category_base' ) : 'category';
	$old_category_base = trim( $old_category_base, '/' );
	$category_rewrite[ $old_category_base . 'infocentr/(.*)$' ] = 'index.php?category_redirect=$matches[1]';

...
while($markers->have_posts()): $markers->the_post();
    $map = json_decode(get_field('map'));
    $address = (get_field('address')) ? '<strong>' . __('Адрес:', 'base') . '</strong> ' . get_field('address') . '<br>' : '' ;
    $phone = (get_field('phone')) ? '<strong>' . __('Телефон:', 'base') . '</strong> ' . get_field('phone') . '<br>' : '' ;
    $mail = (get_field('e-mail')) ? '<strong>' . __('E-mail:', 'base') . '</strong> ' . get_field('e-mail') . '<br>' : '' ;
    $site = (get_field('site')) ? '<strong>' . __('Сайт:', 'base') . '</strong> ' . get_field('site') . '<br>' : '' ;

    $ballon = '
    <h4>' . get_the_title() . '</h4>' .
    $address . $phone . $mail . $site .
    '<strong><a href="' . get_the_permalink($markers->post->ID) . '" class="map-link">Подробнее</a></strong>
    ';

    foreach($map->marks as $mark) {
        //print_r($mark->coords);
        $coords['features'][] = array(
            'type' => 'Feature',
            'id' => $count,
            'geometry' => array(
                'type' => 'Point',
            ...
            'properties' => array(
                ...
                'clusterCaption' => __('Пансионаты', 'base'),
                'hintContent' => __('Для получения дополнительных сведений, кликните на маркер', 'base'),
            ),
        );
        $count++;
    }
